...
void Benes::initialize(int values, int levels, std::vector<uint64_t> &p)
{
									  ...
									  vector<vector<array<osuCrypto::block, 2>>> &ot_output)
{
	...
	if (values == 2)
	{
		if (n == 1)
		{
			...
			block2mpz(src[0], tmp[0]);
			block2mpz(temp_int[0], tmp[1]);
			mpz_sub(tmp[0], tmp[0], tmp[1]);
			mpz_mod(tmp[0], tmp[0], p);
			src[0] = mpz2block(tmp[0]);

			// src[1] = src[1] - temp_int[1];
			block2mpz(src[1], tmp[0]);
			block2mpz(temp_int[1], tmp[1]);
			mpz_sub(tmp[0], tmp[0], tmp[1]);
			mpz_mod(tmp[0], tmp[0], p);
			src[1] = mpz2block(tmp[0]);
	...
	if (values == 3)
	{
		...
		block2mpz(src[2], tmp[0]);
		block2mpz(temp_int[1], tmp[1]);
		mpz_sub(tmp[0], tmp[0], tmp[1]);
		mpz_mod(tmp[0], tmp[0], p);
		src[2] = mpz2block(tmp[0]);
	...
	for (i = 0; i < values - 1; i += 2)
	{
		...
		block2mpz(src[i], tmp[0]);
		block2mpz(temp_int[0], tmp[1]);
		mpz_sub(tmp[0], tmp[0], tmp[1]);
		mpz_mod(tmp[0], tmp[0], p);
		src[i] = mpz2block(tmp[0]);

		// src[i ^ 1] = src[i ^ 1] - temp_int[1]; //  ^ or | ??
		block2mpz(src[i ^ 1], tmp[0]);
		block2mpz(temp_int[1], tmp[1]);
		mpz_sub(tmp[0], tmp[0], tmp[1]);
		mpz_mod(tmp[0], tmp[0], p);
		src[i ^ 1] = mpz2block(tmp[0]);
